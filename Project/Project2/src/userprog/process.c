...
process_execute (const char *file_name) 
{
  ...
  char *save_ptr;
  function_name = malloc(strlen(file_name)+1);
  strlcpy (function_name, file_name, strlen(file_name)+1);
  function_name = strtok_r (function_name," ",&save_ptr);
  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (function_name, PRI_DEFAULT, start_process, fn_copy);
  free(function_name);
  if (tid == TID_ERROR)
    palloc_free_page (fn_copy);
  sema_down(&thread_current()->sema_value);
  return tid;
}
...
process_wait (tid_t child_tid) 
{
  ...
  struct list_elem *child_died=NULL;
  struct list_elem *elem = list_begin (&thread_current()->child_thread); 
  while(elem != list_end (&thread_current()->child_thread))
  {
    struct child *pointer = list_entry (elem, struct child, elem);
    if(pointer->tid == child_tid)
    {
      child = pointer;
      child_died = elem;
      break;
  ...
  thread_current()->child_to_die = child->tid;
  if(!child->died)
    sema_down(&thread_current()->sema_value);
  list_remove(child_died);
  ...
  uint32_t *pd;
    ...
    struct list_elem *elem;
      ...
      while(!list_empty(&thread_current()->file_list))
      {
        ...
        struct thread_open_file *f = list_entry (elem, struct thread_open_file, elem);
